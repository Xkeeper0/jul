<?php
	...
	// 500 posts or 250 posts + 100 days
	$postreq = ($loguser['posts'] >= 500) || ($loguser['posts'] >= 250 && (ctime() - $loguser['regdate']) >= 100*86400);
	// custom titles:  0 = disabled, 1 = auto, 2 = enabled
	if ($loguser['titleoption'] == 0 || $banned) $titleopt = false;
	if ($loguser['titleoption'] == 1 && ($postreq || $power > 0 || $loguser['title'])) $titleopt = true;
	if ($loguser['titleoption'] == 2) $titleopt = true;
	...
	if (!$action) {
		...
		// init empty choices
		$checked1	= array_fill(0, 10, "");
		$checked2	= array_fill(0, 10, "");
		$checked3	= array_fill(0, 10, "");
		$checked4	= array_fill(0, 10, "");
		$checked5	= array_fill(0, 10, "");
		$checked6	= array_fill(0, 10, "");
		...
		$sexlist = "
			<label>$radio=sex value=0 $checked1[0]> <strong style='color: #". getnamecolor(0, $loguser['powerlevel'], false) ."'>Male</strong></label> &nbsp;&nbsp;
			<label>$radio=sex value=1 $checked1[1]> <strong style='color: #". getnamecolor(1, $loguser['powerlevel'], false) ."'>Female</strong></label> &nbsp;&nbsp;
			<label>$radio=sex value=2 $checked1[2]> <strong style='color: #". getnamecolor(2, $loguser['powerlevel'], false) ."'>Other / N/A</strong></label>
			";
		...
		$vsig = "
			<label>$radio=viewsig value=0 $checked2[0]> Disabled</label> &nbsp;&nbsp;
			<label>$radio=viewsig value=1 $checked2[1]> Enabled</label> &nbsp;&nbsp;
			<label>$radio=viewsig value=2 $checked2[2]> Auto-updating</label>
			";
		// $vtool = "
		// 	<label>$radio=posttool value=0 $checked3[0]> Disabled</label> &nbsp;&nbsp;
		// 	<label>$radio=posttool value=1 $checked3[1]> Enabled</label>
		//	 ";
		$pagestyle = "
			<label>$radio=pagestyle value=0 $checked5[0]> Inline</label> &nbsp;&nbsp;
			<label>$radio=pagestyle value=1 $checked5[1]> Seperate line</label>
			";
		$pollstyle = "
			<label>$radio=pollstyle value=0 $checked6[0]> Normal</label> &nbsp;&nbsp;
			<label>$radio=pollstyle value=1 $checked6[1]> Influence</label>
			";
		...
		$seplist = "";
		for ($i = 0; $sepn[$i] ?? null; $i++) {
			$sel = ($i == $loguser['signsep'] ? ' selected' : '');
			$seplist .= "<option value=$i$sel>$sepn[$i] (". ($used[$i] ?? 0) .")";
		}
		...
		while ($set = $sql->fetch($rsets)) {
			...
			$rsetlist .= "<option value=$set[id]$sel>$set[name] (". ($set['used'] ?? 0) .")";
		}
...
//	$tccell2l>$vtool<tr>
	...
	$bio=preg_replace("'<iframe'si", '&lt;iframe', $bio);
		...
		$birthday=0;
		if($bmonth && $bday && $byear) $birthday = @mktime(12, 0, 0, $bmonth, $bday, $byear);
		if(!$icq) $icq=0;
