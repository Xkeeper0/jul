...
#	die();
	...
	if ($_POST['edit'] ?? false) {
		$q	=
			...
			"`hidden` = '". ($hiddeneditok ? intval($_POST['hidden']) : 0) ."', ".
			"`coins` = '". intval($_POST['coins']) ."', ".
			"`gcoins` = '". intval($_POST['gcoins']) ."', ";

		foreach($stats as $stat) {
			if ($_POST['m'. $stat] == "m") $_POST[$stat] *= 100;
			$q		.= "`$stat` = '". intval($_POST[$stat] ?? 0) ."', ";
			$stypes	.= $_POST['m'. $stat];
		}
		$q	.= "`stype` = '$stypes'";
...
//	$stats	= array("sHP", "sMP", "sAtk", "sDef", "sInt", "sMDf", "sDex", "sLck", "sSpd");
	...
	if ($_GET['id']) {
		...
		<form method=\"post\" action=\"?cat=1&id=". $_GET['id'] . (($_GET['type'] ?? false) ? "&type=". $_GET['type'] : "") ."\">
		$tblstart
			<tr>
			...
			<tr>
				...
				$tccell1l colspan=3><input type=\"text\" name=\"name\" value=\"". ($item['name'] ?? "") ."\"  style=\"width: 100%;\" maxlength=\"255\">
					". ($hiddeneditok ? "<br><input type=\"checkbox\" id=\"hiddenitem\" name=\"hidden\" value=\"1\"". (($item['hidden'] ?? false) ? " checked" : "") ."> <label for=\"hiddenitem\">Hidden item</label>" : "") ."
					</td>
				$tccellh>Category</td>
				$tccell1l>". linkbar($categories, $item['cat'], 1, "cat") ." / ". linkbar($alltypes, $item['type'] ?? 0, 1, "type") ."</td>
			</tr>
			<tr>
				$tccellh>Desc</td>
				$tccell1l colspan=3><input type=\"text\" name=\"desc\" value=\"". ($item['desc'] ?? "") ."\" style=\"width: 100%;\"></td>
				$tccellh>Effect <small>(wip)</small></td>
				$tccell1l>". linkbar($effects, ($item['effect'] ?? 0), 1, "effect") ."</td>
			</tr>
			...
			<tr>
				...
				$tccellh> Coins </td>
				$tccell1l><input type=\"text\" name=\"coins\" maxlength=\"8\" size=\"10\" value=\"". ($item['coins'] ?? 0) ."\" class=\"right\"> <img src=\"images/coin.gif\" align=\"absmiddle\"></td>
				$tccellh> G.Coins </td>
				$tccell1l><input type=\"text\" name=\"gcoins\" maxlength=\"8\" size=\"10\" value=\"". ($item['gcoins'] ?? 0)."\" class=\"right\"> <img src=\"images/coin2.gif\" align=\"absmiddle\"></td>
			</tr>
	...
	$items	= $sql -> query("
		SELECT
			...
			`users`.`id` as uid,
			`users`.`sex` as usex,
			`users`.`powerlevel` as upow,
			`users`.`name` as uname FROM `items`
		LEFT JOIN `users`
			ON `users`.`id` = `items`.`user`
		WHERE `cat` = '$cat'
			". (($_GET['type'] ?? false) ? " AND `type` = '". $_GET['type'] ."' " : "")."
...
*/
		...
		$typerow[$item['type']] .= "<tr>
				$tccell1s><a href=\"?cat=$cat&id=". $item['id'] . (($_GET['type'] ?? false) ? "&type=". $_GET['type'] : "") ."\">Edit</a></td>
				$tc2>". $item['uname'] ."</td>$tc2l>". $item['name'] ."</td>";

		$val	= 0;
		foreach($stats as $n => $stat) {
			$num	= ($stype[$n] == "m" ? sprintf('%1.2fx',$item[$stat]/100) : $item[$stat]);
